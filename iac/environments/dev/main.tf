terraform {
  backend "s3" {
    ...
    key            = "hgts-dev-terraform.tfstate"
    region         = "ap-south-2"
...
module "hgts_dev" {
  ...
  hgts_web_data_subnets_2      = data.terraform_remote_state.hgts_preprod_vpc.outputs.data_subnet_2
  ...
  rds_subnet_group        = data.terraform_remote_state.hgts_preprod_vpc.outputs.data_subnet_1
  rds_admin_sg            = data.terraform_remote_state.hgts_preprod_vpc.outputs.linux_sg # Updated to use remote state
