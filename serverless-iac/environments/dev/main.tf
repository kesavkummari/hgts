...
module "hgts_web_prod" {
  ...
  hgts_web_admin_windows_sg_id = data.terraform_remote_state.shared_prod_vpc.outputs.windows_sg_id
  ...
  rds_subnet_group            = data.terraform_remote_state.shared_prod_vpc.outputs.db_subnet_group
  rds_admin_sg                = data.terraform_remote_state.shared_prod_vpc.outputs.postgres_sg_id  # Updated to use remote state
  rds_engine_type             = "postgres"
  rds_engine_version          = "15.4"  # Use latest supported version as per AWS
...
#   # RDS Instance Inputs
#   rds_instance_identifier     = "hgts-dev"
